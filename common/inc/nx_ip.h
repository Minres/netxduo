...
#define NX_IP_ID                     ((UINT32)0x49502020)



/* Define the mask for the IP header length field.  */

#define NX_IP_LENGTH_MASK            ((UINT32)0x0F000000)       /* Mask for length bit      */



/* Define IP event flags.  These events are processed by the IP thread. */

#define NX_IP_ALL_EVENTS             ((UINT32)0xFFFFFFFF)       /* All event flags              */
#define NX_IP_PERIODIC_EVENT         ((UINT32)0x00000001)       /* Periodic event               */
#define NX_IP_UNFRAG_EVENT           ((UINT32)0x00000002)       /* Unfragment event             */
#define NX_IP_ICMP_EVENT             ((UINT32)0x00000004)       /* ICMP message event           */
#define NX_IP_RECEIVE_EVENT          ((UINT32)0x00000008)       /* IP receive packet event      */
#define NX_IP_ARP_REC_EVENT          ((UINT32)0x00000010)       /* ARP deferred receive event   */
#define NX_IP_RARP_REC_EVENT         ((UINT32)0x00000020)       /* RARP deferred receive event  */
#define NX_IP_IGMP_EVENT             ((UINT32)0x00000040)       /* IGMP message event           */
#define NX_IP_TCP_EVENT              ((UINT32)0x00000080)       /* TCP message event            */
#define NX_IP_FAST_EVENT             ((UINT32)0x00000100)       /* Fast TCP timer event         */
#ifdef NX_DRIVER_DEFERRED_PROCESSING
#define NX_IP_DRIVER_PACKET_EVENT    ((UINT32)0x00000200)       /* Driver Deferred packet event */
#endif /* NX_DRIVER_DEFERRED_PROCESSING */
#define NX_IP_IGMP_ENABLE_EVENT      ((UINT32)0x00000400)       /* IGMP enable event            */
#define NX_IP_DRIVER_DEFERRED_EVENT  ((UINT32)0x00000800)       /* Driver deferred processing   */
                                                               /*   event                      */
#define NX_IP_TCP_CLEANUP_DEFERRED   ((UINT32)0x00001000)       /* Deferred TCP cleanup event   */
#ifdef NX_IPSEC_ENABLE
#define NX_IP_HW_DONE_EVENT          ((UINT32)0x00002000)       /* HW done event                */
#endif /* NX_IPSEC_ENABLE */
#define NX_IP_LINK_STATUS_EVENT      ((UINT32)0x00004000)       /* Link status change event     */
...
USHORT _nx_ip_checksum_compute(NX_PACKET *packet_ptr, UINT32 protocol, UINT data_length,
                               UINT32 *_src_ip_addr, UINT32 *_dest_ip_addr);
UINT   _nx_ip_interface_address_mapping_configure(NX_IP *ip_ptr, UINT interface_index, UINT mapping_needed);
UINT   _nx_ip_interface_capability_get(NX_IP *ip_ptr, UINT interface_index, UINT32 *interface_capability_flag);
UINT   _nx_ip_interface_capability_set(NX_IP *ip_ptr, UINT interface_index, UINT32 interface_capability_flag);
UINT   _nx_ip_interface_info_get(NX_IP *ip_ptr, UINT interface_index, CHAR **interface_name, UINT32 *ip_address,
                                 UINT32 *network_mask, UINT32 *mtu_size, UINT32 *phsyical_address_msw,
                                 UINT32 *physical_address_lsw);
UINT _nx_ip_interface_mtu_set(NX_IP *ip_ptr, UINT interface_index, UINT32 mtu_size);
UINT _nx_ip_interface_physical_address_get(NX_IP *ip_ptr, UINT interface_index, UINT32 *physical_msw, UINT32 *physical_lsw);
UINT _nx_ip_interface_physical_address_set(NX_IP *ip_ptr, UINT interface_index, UINT32 physical_msw, UINT32 physical_lsw, UINT update_driver);
UINT _nx_ip_interface_status_check(NX_IP *ip_ptr, UINT interface_index, UINT32 needed_status, UINT32 *actual_status,
                                   UINT32 wait_option);
...
UINT _nxd_ip_raw_packet_source_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, NXD_ADDRESS *destination_ip, UINT address_index, UINT32 protocol, UINT ttl, UINT32 tos);
...
UINT _nx_ip_status_check(NX_IP *ip_ptr, UINT32 needed_status, UINT32 *actual_status, UINT32 wait_option);
...
UINT _nxe_ip_driver_interface_direct_command(NX_IP *ip_ptr, UINT command, UINT interface_index, UINT32 *return_value_ptr);
...
UINT _nxe_ip_info_get(NX_IP *ip_ptr, UINT32 *ip_total_packets_sent, UINT32 *ip_total_bytes_sent,
                      UINT32 *ip_total_packets_received, UINT32 *ip_total_bytes_received,
                      UINT32 *ip_invalid_packets, UINT32 *ip_receive_packets_dropped,
                      UINT32 *ip_receive_checksum_errors, UINT32 *ip_send_packets_dropped,
                      UINT32 *ip_total_fragments_sent, UINT32 *ip_total_fragments_received);
...
UINT _nxde_ip_raw_packet_source_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, NXD_ADDRESS *destination_ip, UINT address_index, UINT32 protocol, UINT ttl, UINT32 tos);
UINT _nxe_ip_status_check(NX_IP *ip_ptr, UINT32 needed_status, UINT32 *actual_status,
                          UINT32 wait_option);
...
UINT _nxe_ip_interface_capability_set(NX_IP *ip_ptr, UINT interface_index, UINT32 interface_capability_flag);
...
UINT _nxe_ip_interface_status_check(NX_IP *ip_ptr, UINT interface_index, UINT32 needed_status,
                                    UINT32 *actual_status, UINT32 wait_option);
UINT _nxe_ip_raw_packet_filter_set(NX_IP *ip_ptr,
                                   UINT (*raw_packet_filter)(NX_IP *, UINT32, NX_PACKET *));
UINT _nxe_ip_raw_receive_queue_max_set(NX_IP *ip_ptr, UINT32 queue_max);
...
IP_DECLARE  UINT32 _nx_ip_created_count;
