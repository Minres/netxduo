...
UINT _nx_nd_cache_add(NX_IP *ip_ptr, UINT32 *dest_ip, NX_INTERFACE *if_index, CHAR *mac, INT IsStatic, INT status, NXD_IPV6_ADDRESS *nxd_interface, ND_CACHE_ENTRY **entry);
...
UINT _nx_nd_cache_find_entry(NX_IP *ip_ptr, UINT32 *dest_ip, ND_CACHE_ENTRY **entry);

/* Find cache entry based on neighbor IP address.  If not found, create new entry. */
UINT _nx_nd_cache_add_entry(NX_IP *ip_ptr, UINT32 *dest_ip, NXD_IPV6_ADDRESS *nxd_address, ND_CACHE_ENTRY **entry);

/* Find cache entry based on LLA. */
UINT _nx_nd_cache_find_entry_by_mac_addr(NX_IP *ip_ptr, UINT32 physical_msw, UINT32 physical_lsw, ND_CACHE_ENTRY **entry);
...
UINT _nxd_nd_cache_entry_set(NX_IP *ip_ptr, UINT32 *dest_ip, UINT if_index, CHAR *mac);
UINT _nxde_nd_cache_entry_set(NX_IP *ip_ptr, UINT32 *dest_ip, UINT if_index, CHAR *mac);

/* Invalidate the whole cache. */
UINT _nxd_nd_cache_invalidate(NX_IP *ip_ptr);
UINT _nxde_nd_cache_invalidate(NX_IP *ip_ptr);

UINT _nxd_nd_cache_ip_address_find(NX_IP *ip_ptr, NXD_ADDRESS *ip_address, UINT32 physical_msw, UINT32 physical_lsw, UINT *if_index);
UINT _nxde_nd_cache_ip_address_find(NX_IP *ip_ptr, NXD_ADDRESS *ip_address, UINT32 physical_msw, UINT32 physical_lsw, UINT *if_index);

UINT _nxd_nd_cache_hardware_address_find(NX_IP *ip_ptr, NXD_ADDRESS *ip_address, UINT32 *physical_msw, UINT32 *physical_lsw, UINT *if_index);
UINT _nxde_nd_cache_hardware_address_find(NX_IP *ip_ptr, NXD_ADDRESS *ip_address, UINT32 *physical_msw, UINT32 *physical_lsw, UINT *if_index);
